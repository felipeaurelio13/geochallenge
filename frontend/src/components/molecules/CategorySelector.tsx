...
export function CategorySelector({ categories, selected, onSelect, ariaLabel }: CategorySelectorProps) {
  return (
    ...
    <div className="relative">
      ...
      >
        {categories.map((cat) => (
          <button
            ...
            className={`pressable menu-category-selector flex-none flex flex-col items-center justify-center w-[72px] min-h-[4.5rem] rounded-md px-1 py-2.5 border transition-colors sm:w-auto sm:flex-1 ${
              selected === cat.id
                ? (cat.accentClass ?? 'border-primary bg-primary/10 text-primary')
                : 'border-app-border bg-app-surface text-app-secondary hover:border-primary/50 hover:text-app-text hover:bg-app-muted'
            }`}
          >
            {cat.icon ? <span aria-hidden="true" className="menu-category-selector__icon text-xl leading-none">{cat.icon}</span> : null}
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-y-0 right-0 w-6 bg-app-bg sm:hidden"
      />
    </div>
  );
}
