...
export function OptionButton({
  ...
  const getButtonClasses = () => {
    const baseClasses =
      'w-full rounded-2xl text-left transition-all duration-200 flex items-start gap-3 overflow-hidden border-2 px-3 py-3 min-h-[58px] sm:items-center sm:px-4 sm:py-3.5 sm:min-h-[64px]';
  ...
  return (
    ...
    >
      <span
        className={`h-9 w-9 shrink-0 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <span className="min-w-0 text-base font-medium leading-tight [overflow-wrap:anywhere] sm:text-lg md:text-xl">
          {option}
        </span>
